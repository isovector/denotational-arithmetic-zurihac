...
... | (k , _) = let
  ...
  ≡⟨ cong! (sym $ toℕ-combine (μ ehjhi) (μ ehj)) ⟩
    ...
    size * size * (size * toℕ (μ k0)) + size * size * (size * toℕ (μ lighi) + toℕ (μ liglo)) + size * (toℕ (zero {2}) + toℕ (μ e) + toℕ (μ h) + toℕ (μ j)) + toℕ (μ f)                                             ≡⟨ cong! (sym $ toℕ-combine (μ lighi) (μ liglo)) ⟩
    size * size * (size * toℕ (μ k0)) + size * size * toℕ (combine (μ lighi) (μ liglo)) + size * (toℕ (zero {2}) + toℕ (μ e) + toℕ (μ h) + toℕ (μ j)) + toℕ (μ f)                                                  ≡⟨⟩
    size * size * (size * toℕ (μ k0)) + size * size * toℕ (uncurry combine (P.map μ μ (lighi , liglo))) + size * (toℕ (zero {2}) + toℕ (μ e) + toℕ (μ h) + toℕ (μ j)) + toℕ (μ f)                                  ≡⟨ cong (\ φ → size * size * (size * toℕ (μ k0)) + size * size * toℕ (uncurry combine (P.map μ μ φ)) + size * (toℕ (zero {2}) + toℕ (μ e) + toℕ (μ h) + toℕ (μ j)) + toℕ (μ f)) $ sym -add-lig-eq ⟩
    ...
    size * size * (size * toℕ (μ k0)) + size * size * toℕ (uncurry combine (P.map μ μ added-lig)) + size * (toℕ (zero {2}) + toℕ (μ e) + toℕ (μ h) + toℕ (μ j)) + toℕ (μ f)                                        ≡⟨ cong!  -add3-2-proof ⟩
